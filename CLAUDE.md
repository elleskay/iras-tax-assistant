...
13. **Turbopack cannot bundle packages with inlined WASM** (e.g. the QuickJS singlefile sandbox variant): `next build` fails with an octal-escape SyntaxError. Add them to `serverExternalPackages` in `next.config.ts`; OpenNext ships externals with the server function. See `docs/DEPLOY.md` #15.
